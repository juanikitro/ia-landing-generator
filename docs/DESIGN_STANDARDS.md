...
6. **Conversión primero, mobile-first por defecto.** CTA `tel:` arriba del fold, barra sticky de llamada en mobile, teléfono tratado como elemento gráfico, link "Cómo llegar" a Google Maps, horarios siempre visibles, prueba social (rating + cantidad de reseñas) en el hero. El celular manda el orden y el ritmo inicial; desktop debe recomponer esa intención en una pieza de conversión más fuerte, no solo estirar columnas.
...
8. **Profundidad y motion con intención.** Preferir, sin volverlo obligatorio, diseños con capas, profundidad, overlays, sombras expresivas, microinteracciones, reveals escalonados e incluso parallax suave cuando refuercen el rubro y la conversión. `prefers-reduced-motion` siempre respetado; sin JS pesado, CDNs ni efectos que compitan con el CTA.
